    ...
    public partial class Form1 : Form
    {
        ...
        private void btnBuscar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                try
                {
                    Paciente encontrado = gestor.ConsultarPorId(textoBusqueda);
                    data_Lista.DataSource = null;
                    data_Lista.DataSource = new List<Paciente> { encontrado };
                    return; // Si lo encuentra por ID, termina la ejecución aquí
                }
                catch (PacienteNoEncontradoException)
                {

                    var resultadosNombre = gestor.ConsultarPorNombre(textoBusqueda);

                    if (resultadosNombre != null && resultadosNombre.Count > 0)
                    {
                        data_Lista.DataSource = null;
                        data_Lista.DataSource = resultadosNombre;
                    }
                    else
                    {
                        MessageBox.Show("No se encontró ningún paciente con ese ID o nombre.", "Búsqueda", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        ActualizarTabla();
                    }
                }
            }
